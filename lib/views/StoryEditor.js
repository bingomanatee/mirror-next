  ...
  return (
    <AccordionPanel label={`${story.title} (${story.id || 'NEW'})`} key={story.identity}>
      <h2>
        ...
        <Button plain={false} primary={true} className={(story.id ? '' : classes['save-extra'])}
          onClick={sem.$do.save}>
          Save Story
        </Button>
      ...
      <div className={classes.editor__formRow + ' ' + classes.editor__formRow2}>
        ...
        <Box direction="row" gap="medium" fill="horizontal">
          ...
          <TextInput value={sem.$my.primaryImage || ''}
      ...
      {
        sem.$my.story.text.map((text, idx) => (
          <EditStoryLine key={text.identity} sem={sem} story={story} idx={idx} text={text.text}/>
        ))
      }
