  ...
  return (
    <AccordionPanel label={`${story.title} (${story.id || 'NEW'})`} key={story.id}>
      ...
      <div className={classes.editor__formRow}>
        ...
        <TextInput type="text" id="title" value={sem.$my.title || ''}
          onChange={sem.$do.change('title')}/>
      </div>
      <div className={classes.editor__formRow + ' ' + classes.editor__formRow2}>
      <label>{sem.$my.primaryImage ? <img style={{width: '12rem', height: '9rem'}} src={sem.$my.primaryImage} /> : 'Primary Image:'}</label>
      <Box direction="row" gap="medium" fill="horizontal">
        <Button primary={false} plain={false} onClick={() => sem.$do.fetchImageFor('primary')}> <Image/></Button>
      ...
      <div className={classes.editor__formRow}>
        ...
        <input type="number" id="title" value={sem.$my.order || 0}
