  ...
  return new MirrorCollection({
    ...
    actions: {
      ...
      playNextStory(self, storyId) {
        console.log('---playNextStory -- story after ', storyId, 'in ', storyMirror.$my.stories)
        if (self.$my.currentStoryId === storyId) {
          self.$do.next();
        }
      },
       ...
       */
      lineImage (self) {
        const line = self.$do.currentLine(false);
        console.log('-;------- line image --- line=');
        console.log(line);
        if (line && line.image) return line.image;
        return self.$do.primaryImage();
      },
      currentStory(self) {
      ...
      currentLine(self, toString = true) {
        const text = self.$do.currentText(toString);
